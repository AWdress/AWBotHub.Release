...
- 定时回复（custom_auto_reply）：原只用第一个账号发送，现遍历所有已连接账号逐个发送，每账号单独通知（带账号标识），单账号失败不影响其他
- 关键词回复：冷却键由 `(user_id, rule_id)` 改为 `(account_id, user_id, rule_id)`，修复一个账号触发后阻塞其他账号、导致只有一个账号回复的问题
- app.py 新增 `get_user_apps()` 返回所有已连接账号

**自动报时昵称**

- 修复多账号 bug：原只改第一个账号昵称，现遍历所有已连接账号各自随机 emoji 报时
- 开关入口从「系统维护→任务控制」移到「功能配置」菜单，更易找到
- 「朱雀自动释放」开关移到「朱雀功能」菜单
- 新增 schedtoggle 回调：在所在菜单内即时切换开关并重新初始化任务，无需跳转
- 新增自定义报时设置（功能配置→报时设置 或 `.autochangename`）：
  - 报时格式模板，支持 `{emoji}{H}{M}{S}{date}{md}{week}` 占位符
  - 可选改 姓/名/姓+名（last_name/first_name/both）
  - 可调报时间隔（1~60 分钟，原写死 5 分钟）

**自动换头像（新功能）**

- 定时把用户账号头像换成图片池里随机一张（换的是用户账号，不是 bot）
- 每账号独立图片池：在 bot 私聊「选择账号 X」进入上传模式，发图存入该账号专属池
- 入口：功能配置→🖼️ 头像设置 或 `.autoavatar`；开关在功能配置菜单
- 可设换头像间隔（最小 10 分钟防限流）、是否删旧头像（仅删本功能上次设的那张，不动原有头像）、每账号清空
- 图片存 `assets/avatars/<账号名>/`（已在 .gitignore 忽略）

**修复 Pyrogram 处理器 group 冲突**

- Pyrogram 每个 group 只跑首个匹配处理器；原 hdsky、keyword 文字输入器同挤 group=-2 互相阻塞
- 将各配置菜单文字输入器分到唯一 group（-1~-8），互不截断
